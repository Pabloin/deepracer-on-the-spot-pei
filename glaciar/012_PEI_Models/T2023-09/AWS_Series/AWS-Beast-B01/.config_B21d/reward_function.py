...
class Reward:
    ...
    @staticmethod
    def fn_curvas_tres_y_seis(params, SPEED_DESEADA_DEFAULT=1.25):
        ...
        if isZonaCurvaTres or isZonaCurvaSeis:
            ...
            if speed_deseada >= SPEED_DESEADA_DEFAULT:
...
def reward_function(params):
    ...
    reward_curvas_3y6_speed = Reward.fn_curvas_tres_y_seis(params, 1.40)


    # Curva 3y6 sobre la izquierda 

    # Mar recompensa en Cruva Tres y Seis
    isZonaCurvaTres   = Track.isz(CURVA_03_LL_ZONA, next_wp)
    isZonaCurvaSeis   = Track.isz(CURVA_06_LL_ZONA, next_wp)

    reward_curvas_3y6LR = 1.0
    if isZonaCurvaTres or isZonaCurvaSeis:
        reward_curvas_3y6LR = Reward.fn_curvas(params)

    # reward += reward_curvas + reward_rectas + reward_curvas_3y6

    reward += reward_curvas_3y6_speed + reward_curvas_3y6LR
